...
3. Execute the script 'cyclistic/scripts/00_init.R' to prepare the environment for analysis
...
Make relevant character columns lower case: 
```{r}
trips <- trips %>% 
...
print(trip_summary)
```
As shown in the table above, there are negative trip durations in the classic_bike and electric_bike, suggesting data inaccuracies, and docked_bike shows abnormally long durations, skewing the overall data distribution.
...
```
Extremely long durations, a significantly lower ride count, and exclusive usage by casual users are anomalies exclusively observed in docked bikes, collectively suggesting a consistent pattern rather than random noise. Upon further investigation, we determined that docked_bikes represent bikes taken out of circulation. Therefore, they will be excluded from our dataset for the purposes of this analysis.
...
We will use the log10 scale once again to delve deeper into the trip durations of classic and electric bikes, aiming to further refine our dataset: 
```{r, fig.width=8, fig.height=4}
ggplot(trips, aes(x = trip_duration)) +
...
Finally, let's look trip duration distribution once again after it's been cleaned up: 
```{r, fig.width=8, fig.height=4}
ggplot(trips, aes(x = trip_duration)) +
  geom_histogram(aes(fill = rideable_type), bins = 50, alpha = 0.7) +
  facet_wrap(~rideable_type) +
...
We can remove rows with NAs in end coordinates. For how insignificant the amount is, we considered them noise. 
```{r}
trips <- trips %>% 
...
##### Start & End Coordinates Precision
...
The next step utilizes add_and_impute_coord_precisions function once again to recalculate and update the precision of start and end point coordinates in trips. 
```{r}
